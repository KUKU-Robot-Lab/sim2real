...
"""palm_cmd — control-only fabric 노드에 palm 목표를 보낸다 (/policy_control/palm_cmd, PoseStamped).
    ...
    python3 deploy/policy_control/tools/palm_cmd.py --rel 0.05 0 0                    # 현재 palm 에서 +5 cm x
    python3 deploy/policy_control/tools/palm_cmd.py --rel 0 0 0 0 0 0.1               # roll pitch yaw 델타 [rad]
    python3 deploy/policy_control/tools/palm_cmd.py --abs 0.30 -0.30 0.40 1.571 0 1.571   # x y z roll pitch yaw
    python3 deploy/policy_control/tools/palm_cmd.py --rel 0 0 0.03 --hold 2.0         # 2 s 동안 10 Hz 재발행
    python3 deploy/policy_control/tools/palm_cmd.py --rel 0.02 0 0 --dry-run          # 계산만, 발행 없음
...
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))     # sim2real/deploy/policy_control → `import policy_control`
