...
#   usage: SIDE=left ROS_DOMAIN_ID=96 deploy/policy_control/tools/fake_direct_run.sh [logdir]
...
case "$PD_CONFIG" in *.yaml) PLANT_PD="$PD_CONFIG" ;; *) PLANT_PD="deploy/policy_control/config/pd_${PD_CONFIG}.yaml" ;; esac
launch_bg ros2 launch deploy/policy_control/launch/fake_plant.launch.py side:=$SIDE robot:=$ROBOT contract:=$CONTRACT pd_config:=$PLANT_PD plant_model:=${PLANT_MODEL:-pd} > "$LOG/fake_plant.log" 2>&1
sleep 3
launch_bg ros2 launch deploy/policy_control/launch/pd_controller.launch.py contract:=$CONTRACT robot:=$ROBOT pd_config:=$PD_CONFIG sides:=$SIDE execute:=true stage:=${PD_STAGE:-full} fake:=true use_source:=true > "$LOG/pd.log" 2>&1
launch_bg ros2 launch deploy/policy_control/launch/policy_chain.launch.py contract:=$CONTRACT robot:=$ROBOT side:=$SIDE device:=cuda:0 fake:=true use_source:=true > "$LOG/chain.log" 2>&1
wait_status 3
...
if [ $RC -eq 0 ]; then
  ...
  python deploy/policy_control/tools/palm_cmd.py --rel 0 0 0 --dry-run 2>&1 | tee "$LOG/palm_before.txt"
  python deploy/policy_control/tools/palm_cmd.py --rel 0 0 $DZ --hold 1.0 2>&1 | tee "$LOG/palm_cmd.txt" || RC=1
  sleep 8
  python deploy/policy_control/tools/palm_cmd.py --rel 0 0 0 --dry-run 2>&1 | tee "$LOG/palm_after.txt"
  python deploy/policy_control/tools/hand_cmd.py --contract $CONTRACT --side $SIDE --close $CLOSE --hold 1.0 > "$LOG/hand_close.txt" 2>&1 || RC=1
  sleep 5
  python deploy/policy_control/tools/hand_cmd.py --contract $CONTRACT --side $SIDE --close 0 --hold 1.0 > "$LOG/hand_open.txt" 2>&1 || RC=1
  sleep 4
