...
"""hand_cmd — control-only fabric 노드에 손 관절 목표를 보낸다 (/policy_control/hand_cmd, JointState canonical).
    ...
    python3 deploy/policy_control/tools/hand_cmd.py --contract logs/policy/asset_openarm_dg5f-m_bi_rl/deploy_contract.json \
        --side left --close 0.0                       # 계약 open pose (home_hand)
    python3 deploy/policy_control/tools/hand_cmd.py --contract … --side right --close 0.6      # open→grip 60 %
    python3 deploy/policy_control/tools/hand_cmd.py --contract … --side right --close 0 --joint r_hj_index_2=0.8
    python3 deploy/policy_control/tools/hand_cmd.py --contract … --side left --close 1 --dry-run
