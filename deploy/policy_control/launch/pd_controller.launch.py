"""pd 노드 단독 launch — 상류(obs/policy/fabric)가 죽어도 pd 가 같이 내려가지 않도록 분리한다.
    ...
    ros2 launch policy_control pd_controller.launch.py contract:=logs/policy/left_v2B25/deploy_contract.json \
        robot:=left_gripper_fake pd_config:=deploy/policy_control/config/pd_left.yaml fake:=true execute:=false
...
인자
  ...
  use_source   true 면 venv python 으로 deploy/policy_control/policy_control/pd_node.py 실행
