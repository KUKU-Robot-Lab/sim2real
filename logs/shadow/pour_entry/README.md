...
| `stream_left_v2b25.npz` / `stream_right_e1_v2.npz` | 각 정책의 자기 env 폐루프 기록 (v2 스키마: actions·obs·지령·실측·gate/latch). 러너의 goal·스폰·재현 대조 기준. 회귀: `sim2real/tests/test_stream_consistency.py` |
| `state_left_end_v2.json` / `state_right_end_v2.json` | 파지 종료 상태 (실측+지령 — 지령이 파지력) |

## 폐기 (미러 시대 — 물리 우회, 교훈 보존용)

> 2026-09-22: 이 절의 GIF 와 분류되지 않은 GIF 는 **git 추적만 뗐다**(디스크에는 남아 있다). 정본 `bimanual_closedloop.gif` 만 추적한다.
