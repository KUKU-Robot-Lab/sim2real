...
def test_absolute_encoder_degrees_map_to_ticks():
    assert deg_to_tick(0.0) == 0
    assert deg_to_tick(90.0) == 1024
    assert deg_to_tick(180.0) == 2048
    assert deg_to_tick(360.0) == 4095
    assert tick_to_deg(4095) == pytest.approx(360.0)
...
def test_save_calibration_yaml_writes_stable_file(tmp_path: Path):
    ...
    save_calibration_yaml(
        ...
        baud=1_000_000,
        motors=[
            MotorCalibration("pan", 1, 0.0, 360.0, False),
            MotorCalibration("tilt", 2, 40.0, 140.0, True),
        ],
    )
