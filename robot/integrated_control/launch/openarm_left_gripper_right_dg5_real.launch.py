...
# robot/integrated_control/launch/ → robot/. urdf/ 와 integrated_control/ 은 둘 다 robot/ 아래다.
# ⚠ openarm_left_gripper_bimanual_real.xacro 는 이 저장소에 한 번도 커밋된 적이 없다(원래부터의 결함, 2026-09-22 확인).
ROBOT_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
OPENARM_XACRO_PATH = os.path.join(
    ROBOT_DIR, "urdf", "openarm_left_gripper_bimanual_real.xacro"
)
OPENARM_CONTROLLERS_PATH = os.path.join(
    ROBOT_DIR, "integrated_control", "config", "openarm_left_gripper_bimanual_controllers.yaml"
)
