...
핵심 노드는 `robot/isaacsim_bridge/isaacsim_bridge/bridge_node.py` 하나다. 이 노드가
...
`bridge_node`가 아니라 전용 노드 `robot/isaacsim_bridge/isaacsim_bridge/rh56f1_hand_bridge_node.py`가 담당한다.
...
순열은 `rh56f1_hand.py`(순수 로직)가 처리하고, 값은 `robot/isaacsim_bridge/config/rh56f1_hand_calibration.yaml`로
...
python3 sim2real/legacy/scripts/deprecated/sim2real_dryrun.py
```
...
| 문서 | 내용 |
|---|---|
| `robot/isaacsim_bridge/README.md` | 브리지 패키지 상세, 튜닝 리포트 |
| `robot/isaacsim_bridge/ISAACSIM_ACTION_GRAPH.md` | Isaac Sim 쪽 Action Graph로 5토픽 발행 |
| `robot/isaacsim_bridge/ISAACSIM_POLICY_WIRING.md` | Action Graph에 정책 출력 연결 |
