...
순서로 실행하는 런북이다. 개념·토픽 표·설계 배경은 `robot/ROBOT_ISAACSIM_CONNECTION.md`,
브리지 내부는 `robot/isaacsim_bridge/README.md`를 본다. 이 문서는 "무슨 명령을 어떤 순서로
치는가"만 다룬다.
...
- Isaac Sim 쪽은 Action Graph로 `/isaacsim/*` 토픽을 주고받는다
  (`robot/isaacsim_bridge/ISAACSIM_ACTION_GRAPH.md`). 실물 없이 배선만 볼 때는 §5 dry-run.
...
라디안↔레지스터 변환값은 `robot/isaacsim_bridge/config/rh56f1_hand_calibration.yaml`.
...
python3 -m pytest tests/test_cup_pose_relay.py -q
```
...
| `robot/ROBOT_ISAACSIM_CONNECTION.md` | 로봇별 연동 상세, 토픽 표, 설계 배경 |
| `robot/isaacsim_bridge/README.md` | 브리지 파라미터·튜닝·Action Graph |
